...
public class VitessResultSetTest extends BaseTest {
    ...
    @Test public void testGetStringUsesEncoding() throws Exception {
        VitessStatement statement = getVitessStatement();
        VitessResultSet resultOne = PowerMockito.spy(new VitessResultSet(getCursorWithRows(), statement));
        resultOne.next();
        ...
        PowerMockito.verifyPrivate(resultOne, VerificationModeFactory.times(1)).invoke("convertBytesToString", Matchers.any(byte[].class), Matchers.any(FieldWithMetadata.class));

        statement.getConnection().setIncludedFields(Query.ExecuteOptions.IncludedFields.TYPE_AND_NAME);
        VitessResultSet resultTwo = PowerMockito.spy(new VitessResultSet(getCursorWithRows(), statement));
        resultTwo.next();
        ...
        PowerMockito.verifyPrivate(resultTwo, VerificationModeFactory.times(0)).invoke("convertBytesToString", Matchers.any(byte[].class), Matchers.any(FieldWithMetadata.class));
    }
    ...
    @Test public void testGetObjectForBitValues() throws Exception {
        ...
        VitessResultSet vitessResultSet = PowerMockito.spy(new VitessResultSet(new SimpleCursor(result), statement));
        vitessResultSet.next();
        ...
        statement.getConnection().setIncludedFields(Query.ExecuteOptions.IncludedFields.TYPE_AND_NAME);
        vitessResultSet = PowerMockito.spy(new VitessResultSet(new SimpleCursor(result), statement));
        vitessResultSet.next();
