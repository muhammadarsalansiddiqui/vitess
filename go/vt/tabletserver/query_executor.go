...
func (qre *QueryExecutor) Stream(excludeFieldNames bool, excludeFieldMetadata bool, sendReply func(*sqltypes.Result) error) error {
	...
	return qre.streamFetch(conn, qre.plan.FullQuery, qre.bindVars, nil, excludeFieldNames, excludeFieldMetadata, sendReply)
}
...
func (qre *QueryExecutor) streamFetch(conn *DBConn, parsedQuery *sqlparser.ParsedQuery, bindVars map[string]interface{}, buildStreamComment []byte, excludeFieldNames bool, excludeFieldMetadata bool, callback func(*sqltypes.Result) error) error {
	sql, err := qre.generateFinalSQL(parsedQuery, bindVars, buildStreamComment)
	if err != nil {
		return err
	}
	return qre.execStreamSQL(conn, sql, excludeFieldNames, excludeFieldMetadata, callback)
}
...
func (qre *QueryExecutor) execStreamSQL(conn *DBConn, sql string, excludeFieldNames bool, excludeFieldMetadata bool, callback func(*sqltypes.Result) error) error {
	start := time.Now()
	err := conn.Stream(qre.ctx, sql, callback, int(qre.qe.streamBufferSize.Get()), excludeFieldNames, excludeFieldMetadata)
